...
Функция ИсправляемыеОшибки()
	...
	Список.Добавить("ОтсутствуютТребуемыеИндексыВТаблицахСОграничениемДоступа",
		НСтр("ru = 'Управление доступом. Отсутствуют требуемые индексы в таблицах с ограничением'"));
...
Процедура Подключаемый_УправлениеДоступом_ПроверитьВнедрение()
	...
	Если ТребуетсяВыполнениеПроверки("ОтсутствуютТребуемыеИндексыВТаблицахСОграничениемДоступа") Тогда
		ПроверитьТребуемыеИндексыВТаблицахСОграничениемДоступа(Параметры,
			ИсправлятьОшибку("ОтсутствуютТребуемыеИндексыВТаблицахСОграничениемДоступа"));
	КонецЕсли;
...
Процедура ПроверитьТребуемыеИндексыВТаблицахСОграничениемДоступа(Параметры, Исправлять)
	
	ТребуемыеИндексы = Параметры.УправлениеДоступом.НастройкиВнедрения.ТребуемыеИндексы;
	
	Для Каждого ОписаниеТаблицы Из ТребуемыеИндексы Цикл
		ОбъектМетаданных = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ОписаниеТаблицы.Ключ);
		Если ОбъектМетаданных = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		ЕстьИзменения = Ложь;
		ПроверитьТребуемыеИндексыВТаблице(ДокументDOM,
			ОбъектМетаданных, ОписаниеТаблицы.Значение, Исправлять, ЕстьИзменения);
...
Процедура ПроверитьТребуемыеИндексыВТаблице(ДокументDOM, ОбъектМетаданных, ТребуемыеИндексы, Исправлять, ЕстьИзменения)
	
	Если Исправлять Тогда
		РезультатXPath = ВычислитьВыражениеXPath(ВыражениеXPathЗначениеСвойстваИндексирование(), ДокументDOM);
		ИндексированиеПолей = Новый Соответствие;
		Пока Истина Цикл
			УзелСвойствоИндексирование = РезультатXPath.ПолучитьСледующий();
			Если УзелСвойствоИндексирование = Неопределено Тогда
				Прервать;
			КонецЕсли;
			ИмяПоля = "";
			Для Каждого УзелСвойство Из УзелСвойствоИндексирование.РодительскийУзел.ДочерниеУзлы Цикл
				Если УзелСвойство.ИмяУзла = "Name" Тогда
					ИмяПоля = УзелСвойство.ТекстовоеСодержимое;
					Прервать;
				КонецЕсли;
			КонецЦикла;
			Если Не ЗначениеЗаполнено(ИмяПоля) Тогда
				Продолжить;
			КонецЕсли;
			ИндексированиеПолей.Вставить(ИмяПоля, УзелСвойствоИндексирование);
		КонецЦикла;
	КонецЕсли;
	
	Для Каждого ТребуемыйИндекс Из ТребуемыеИндексы Цикл
		ИмяПоля = СтрСоединить(ТребуемыйИндекс.Поля, ",");
		Если ТребуемыйИндекс.ЕстьВМетаданных Тогда
			Продолжить;
		КонецЕсли;
		
		ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Отсутствует индексирование поля ""%1"".'"),
			ИмяПоля);
		
		Если Исправлять Тогда
			УзелСвойствоИндексирование = ИндексированиеПолей.Получить(ИмяПоля);
			УзелСвойствоИндексирование.ТекстовоеСодержимое = "Index";
			ЕстьИзменения = Истина;
			
			ПояснениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Исправлено. Для поля ""%1"" включено индексирование для эффективного обновления доступа на уровне записей.'"),
				ИмяПоля);
		Иначе
			ПояснениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Для поля ""%1"" включите индексирование для эффективного обновления доступа на уровне записей.'"),
				ИмяПоля);
		КонецЕсли;
...
Функция ВыражениеXPathЗначениеСвойстваИндексирование()
	
	Если КонфигурацияEDT Тогда
		Возврат "//Indexing";
	Иначе
		Возврат "//xmlns:Indexing";
	КонецЕсли;
